...
; I want the chunks of memory to be 1 byte each
        ...
        .thumb
        .global main
a_p: .word 0x200004F0 ; array pointer (field)
cnt: .equ 10          ; size of the array in bytes (constant)
main:
        .asmfunc
...
loop_i:
        STRB        R2, [R1], #1  ; *R1=R2, R1=R1+1 (1 byte further)
        ADD         R2, #3        ; R2=R2+3
        ADD         R0, #1        ; increment R0 (the loop index)
        CMP         R0, #cnt      ; compare index to total loops needed
        BNE         loop_i        ; if not equal go back to label loop_i
...
loop_s:
        ...
        CMP         R0, #cnt      ; compare index to total loops needed
        BNE         loop_s        ; if not equal go back to label loop_s
    ; R3 now holds 0x000000EB
    ; this is an infinite loop
loop:   mov         R0, R0        ; move low 16 bits of R0 into R0
        B           loop          ; go back to label loop
        .endasmfunc
        .end
